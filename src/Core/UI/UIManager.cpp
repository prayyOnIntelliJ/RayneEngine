...
void UIElement::UpdateDrawables()
{
    ...
    } else if (type == UIElementType::Panel) { shape.setFillColor(color); } else
    {
    ...
    if (type == UIElementType::Text || type == UIElementType::Button)
    {
        ...
        } else { drawableText.setPosition(position); }
    }
}
...
void UIManager::Load(const std::string &path)
{
    ...
    for (auto &j: data["ui_elements"])
    {
        ...
        if (j.contains("color")) el.color = sf::Color(j["color"][0], j["color"][1], j["color"][2],
        ...
        if (el.type == UIElementType::Text || el.type == UIElementType::Button)
        {
            ...
            if (j.contains("textColor")) el.textColor = sf::Color(j["textColor"][0], j["textColor"][1],
                                                                  j["textColor"][2],
                                                                  j["textColor"].size() > 3
        ...
        if (el.type == UIElementType::Button)
        {
            if (j.contains("normalColor")) el.normalColor = sf::Color(j["normalColor"][0], j["normalColor"][1],
                                                                      j["normalColor"][2],
                                                                      j["normalColor"].size() > 3
                                                                          ? j["normalColor"][3].get<int>()
                                                                          : 255);
            if (j.contains("hoverColor")) el.hoverColor = sf::Color(j["hoverColor"][0], j["hoverColor"][1],
                                                                    j["hoverColor"][2],
                                                                    j["hoverColor"].size() > 3
                                                                        ? j["hoverColor"][3].get<int>()
                                                                        : 255);
            if (j.contains("pressedColor")) el.pressedColor = sf::Color(j["pressedColor"][0], j["pressedColor"][1],
                                                                        j["pressedColor"][2],
