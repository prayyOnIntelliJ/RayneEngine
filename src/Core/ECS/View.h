...
public:
    ...
    struct Iterator
    {
        ...
        Iterator(const std::vector<Entity> &e, size_t i, std::tuple<Pool<Components> *...> p)
            : entities(e), index(i), pools(p) { CheckValid(); }

        void CheckValid() { while (index < entities.size() && !IsValid(entities[index])) { index++; } }

        bool IsValid(Entity e) { return (std::get<Pool<Components> *>(pools)->Has(e) && ...); }
